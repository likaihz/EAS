...
N_GENERATION = 150   # training step
LR = 0.01            # learning rate


# fitness function
def get_fitness(pred): 
    return -((pred[:, 0])**2 + pred[:, 1]**2+20-10*(np.cos(2*np.pi*pred[:, 0])+np.cos(2*np.pi*pred[:, 1])))
...
for i in range(n):
    for j in range(n):
        Z[j, i] = get_fitness(np.array([[x[i], x[j]]]))
plt.contourf(X, Y, -Z, 100, cmap=plt.cm.rainbow); plt.xlim(-20, 20);plt.ylim(-20,20);plt.ion()
...
for g in range(N_GENERATION):
    ...
    sca = plt.scatter(kids[:, 0], kids[:, 1], s=30, c='k');plt.pause(0.001)
    plt.savefig('image/'+str(g)+'.png', dpi=300)
